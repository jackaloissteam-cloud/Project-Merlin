...
.mission-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.mission-card{min-height:142px;padding:20px;border:1px solid rgba(255,255,255,.09);border-radius:18px;background:linear-gradient(145deg,rgba(255,255,255,.065),rgba(255,255,255,.018));display:flex;flex-direction:column;justify-content:space-between}.mission-card.primary{background:linear-gradient(145deg,rgba(216,169,91,.2),rgba(255,255,255,.025));border-color:rgba(216,169,91,.35)}.mission-card small,.mission-meta small{font-size:.62rem;letter-spacing:.18em;color:#8f9aa5}.mission-card strong{font-size:clamp(1.65rem,3vw,2.45rem);line-height:1;color:#fff}.mission-card em{font-style:normal;font-size:.58rem;letter-spacing:.14em;color:#78838e}
.mission-meta{display:grid;grid-template-columns:1fr 1fr 1.5fr 1.4fr;margin-top:14px;border:1px solid var(--merlin-line);border-radius:16px;overflow:hidden}.mission-meta>div{padding:15px 16px;border-right:1px solid var(--merlin-line)}.mission-meta>div:last-child{border-right:0}.mission-meta strong{display:block;margin-top:6px;font-size:.82rem;color:#fff;overflow-wrap:anywhere}.mission-message{margin-top:14px;padding:13px 16px;border-left:2px solid var(--merlin-amber);background:rgba(216,169,91,.07);color:#c8cfd5;font-size:.86rem}
.mission-timeline{margin:0 0 26px;padding:26px;border:1px solid var(--merlin-line);border-radius:22px;background:rgba(10,15,21,.82)}.timeline-list{margin-top:18px}.timeline-item{position:relative;display:grid;grid-template-columns:18px 1fr;gap:14px;padding:0 0 22px}.timeline-item:not(:last-child)::before{content:"";position:absolute;left:5px;top:12px;bottom:0;width:1px;background:linear-gradient(var(--merlin-amber),rgba(255,255,255,.08))}.timeline-marker{z-index:1;width:11px;height:11px;margin-top:4px;border:2px solid var(--merlin-amber);border-radius:50%;background:#091018}.timeline-item small{display:block;font-size:.62rem;color:#7e8994}.timeline-item strong{display:block;margin:.25rem 0;color:#fff;font-size:.86rem;letter-spacing:.08em}.timeline-item p{margin:0;color:var(--merlin-muted);font-size:.86rem}.timeline-empty{margin-top:16px;color:var(--merlin-muted)}
@media(max-width:760px){.mission-grid{grid-template-columns:1fr 1fr}.mission-meta{grid-template-columns:1fr 1fr}.mission-center{padding:20px}.mission-center__header{flex-direction:column}.merlin-passport{grid-template-columns:1fr;padding:20px}.merlin-passport__intro{padding:4px 4px 20px;border-right:0;border-bottom:1px solid var(--merlin-line)}.merlin-passport__grid{grid-template-columns:1fr 1fr}.merlin-passport article{min-height:82px;padding:14px}.merlin-hero,.merlin-hero__content{min-height:600px}.merlin-hero__content{padding:48px 24px 26px}.merlin-brand{left:24px;top:24px;font-size:.58rem}.merlin-statusbar{grid-template-columns:1fr 1fr}.merlin-statusbar>div{border-bottom:1px solid var(--merlin-line)}.merlin-hero h1{font-size:3.9rem!important}}
/* MERLIN PREMIUM FINISH */
html{scroll-behavior:smooth}.gradio-container{animation:merlinPageIn .9s cubic-bezier(.2,.8,.2,1) both}
@keyframes merlinPageIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
.merlin-hero__content{animation:heroReveal 1.15s cubic-bezier(.18,.82,.22,1) both}
@keyframes heroReveal{from{opacity:0;transform:translateY(28px)}to{opacity:1;transform:none}}
.merlin-hero::before{animation:ambientShift 12s ease-in-out infinite alternate}
@keyframes ambientShift{from{filter:brightness(.84) saturate(.9)}to{filter:brightness(1.07) saturate(1.08)}}
.merlin-hero.airborne{box-shadow:0 34px 110px rgba(0,0,0,.55),0 0 0 1px rgba(97,217,138,.08)}
.merlin-hero.hangar{box-shadow:0 34px 110px rgba(0,0,0,.55),0 0 0 1px rgba(216,169,91,.12)}
.merlin-version{position:absolute;top:34px;right:64px;font-size:.58rem;letter-spacing:.18em;color:rgba(255,255,255,.38)}
.merlin-scrollcue{display:flex;align-items:center;gap:.75rem;margin:0 0 14px;font-size:.56rem;letter-spacing:.18em;color:rgba(255,255,255,.48)}
.merlin-scrollcue span{display:block;width:34px;height:1px;background:linear-gradient(90deg,var(--merlin-amber),transparent);animation:cuePulse 1.8s ease-in-out infinite}
@keyframes cuePulse{0%,100%{opacity:.35;transform:scaleX(.75);transform-origin:left}50%{opacity:1;transform:scaleX(1)}}
.status-dot,.mission-badge span{animation:statusBreath 2.4s ease-in-out infinite}
@keyframes statusBreath{0%,100%{transform:scale(.92)}50%{transform:scale(1.12)}}
.merlin-passport,.mission-center,.mission-timeline{transition:transform .35s ease,border-color .35s ease,box-shadow .35s ease}
.merlin-passport:hover,.mission-center:hover,.mission-timeline:hover{transform:translateY(-3px);border-color:rgba(216,169,91,.28);box-shadow:0 28px 74px rgba(0,0,0,.36)}
.mission-card,.merlin-passport article{position:relative;overflow:hidden;transition:transform .28s ease,border-color .28s ease,background .28s ease}
.mission-card::after,.merlin-passport article::after{content:"";position:absolute;inset:-100% auto -100% -45%;width:34%;transform:rotate(16deg);background:linear-gradient(90deg,transparent,rgba(255,255,255,.08),transparent);transition:left .7s ease}
.mission-card:hover::after,.merlin-passport article:hover::after{left:125%}.mission-card:hover,.merlin-passport article:hover{transform:translateY(-4px);border-color:rgba(216,169,91,.28)}
.plot-container,.js-plotly-plot{border-radius:22px!important;overflow:hidden!important}.block.gradio-plot{border:1px solid var(--merlin-line)!important;border-radius:24px!important;background:rgba(7,11,15,.86)!important;padding:8px!important;box-shadow:0 22px 60px rgba(0,0,0,.26)}
button{transition:transform .2s ease,box-shadow .2s ease,filter .2s ease!important}button:hover{transform:translateY(-2px);filter:brightness(1.05)}button.primary:hover{box-shadow:0 12px 28px rgba(216,169,91,.2)!important}
.merlin-footer{margin:34px 0 12px;padding:24px 4px 10px;border-top:1px solid var(--merlin-line);display:grid;grid-template-columns:1fr auto;align-items:center;gap:16px;color:var(--merlin-muted)}
...
.prologue-copy{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#f4f1e8;animation:copyFade 6.1s ease forwards}
.prologue-copy small{font-size:.65rem;letter-spacing:.34em;color:#c89f60;margin-bottom:20px}.prologue-copy h1{margin:0!important;font:400 clamp(2.4rem,6vw,5.4rem)/1.05 Georgia,serif!important;letter-spacing:-.035em!important;color:#f5f0e6!important}.prologue-copy p{margin:18px 0 0;color:#9fa8b0;letter-spacing:.12em;font-size:.78rem}
.prologue-aircraft{position:absolute;left:50%;bottom:12%;transform:translateX(-50%);width:min(760px,84vw);height:96px;opacity:0;animation:aircraftReveal 6.2s ease forwards}
.prologue-aircraft::before{content:"";position:absolute;left:14%;right:10%;top:43%;height:18px;border-radius:80% 36% 40% 80%;background:linear-gradient(180deg,#353d44,#11171c 60%,#050708);box-shadow:0 10px 30px rgba(0,0,0,.7)}
.prologue-aircraft::after{content:"";position:absolute;left:31%;top:31%;width:47%;height:9px;border-radius:60%;background:#171d22;transform:skewX(-18deg);box-shadow:0 0 0 1px rgba(255,255,255,.04)}
.aircraft-name{position:absolute;left:44%;top:24%;font:700 .56rem/1 Arial;letter-spacing:.18em;color:rgba(216,169,91,.72)}
.propeller{position:absolute;left:9.5%;top:2%;width:78px;height:78px;border:1px solid rgba(216,169,91,.25);border-radius:50%;animation:propellerSpin 1.2s linear 2.7s infinite}.propeller i{position:absolute;left:37px;top:4px;width:4px;height:70px;border-radius:50%;background:linear-gradient(transparent,rgba(216,169,91,.55),transparent)}.propeller i+ i{transform:rotate(90deg)}
...
.merlin-statement{font-size:clamp(1.15rem,2.1vw,1.8rem)!important;max-width:760px!important}
.merlin-statusbar{max-width:1120px!important;background:rgba(4,8,12,.52)!important;border-color:rgba(255,255,255,.15)!important}
.merlin-passport,.mission-center,.mission-timeline,.gradio-plot,.merlin-footer,.merlin-section-title,.gradio-row,.gradio-accordion{width:min(1320px,calc(100% - 48px))!important;margin-left:auto!important;margin-right:auto!important}
.mission-center{margin-top:48px!important}
.block.gradio-plot{margin-top:28px!important;padding:12px!important;min-height:720px!important;background:linear-gradient(180deg,rgba(11,17,23,.96),rgba(5,9,13,.98))!important}
...
@media(max-width:760px){
 ...
 .merlin-statusbar{grid-template-columns:1fr 1fr!important}
...
/* MAP / PLOT: dark from the first paint to prevent a bright loading flicker */
.gradio-plot,.block.gradio-plot,.plot-container,.js-plotly-plot,
.js-plotly-plot .plotly,.js-plotly-plot .svg-container,.js-plotly-plot .main-svg{
 background:#070b10!important;background-color:#070b10!important;
}
.gradio-plot:empty,.plot-container:empty{background:#070b10!important;}
"""
...
with gr.Blocks(title="Project Merlin – Frances Dell") as demo:
    ...
    # Operations Center directly below the intro so controls remain reachable on mobile.
    with gr.Accordion("Operations Center · Test & Technik", open=False, elem_classes=["merlin-ops-accordion"]):
        ...
        preset = gr.Dropdown(list(PRESETS), value="🛩️ Frances Dell – OO-NZW", label="Flugzeug", interactive=True)
    ...
    # The live presentation follows the controls.
    ...
    outputs = [hero_html, passport_html, mission_html, target_md, map_plot, timeline_html, event_table, footer_html, last_event]
